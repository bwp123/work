...
public class Main5
{
	public final static String homepage="/clt4/home/clt4/sy/syv201309/index.json";
	public final static String channel="/clt4/home/clt4/pd/v201307/index.json";
	public final static String live="/clt4/home/clt4/zb/zbv201307/index.json";
	public final static String prefix = "https://api.tv189.com/v2/Internet?";
	public static final String APP_SECRET = "EAD2E42C09D225B5BA5876A98BEBD5C7";
	...
	public final static String APPID="10160010000";
	
	public static int HOMEPAGE_ID=2;
	public static int CHANNEL_ID=3;
	public static int LIVE_ID=4;
...
//				System.out.println(list.get(i).get("isrecord"));
							   ...
							   if(clickType!=null)
							   {
								   if(clickType.intValue()==0)
								   {
									   ...
									   if(tab.containsKey("liveId"))
									   {
										   ...
										   if(countOccurrence(title+"->"+subname,"->")==2)
										   {
	...
	public static void main(String[] args)
	{
		...
		//遍历首页
		new Node("首页",token,homepage,3).visit(false);
	    List<String> channels=new ArrayList<String>();
	    for(int i=0;i<Node.realtimes.size();i++)
	    {
	    	String fullname=Node.realtimes.get(i)[1];
	    	Node.realtimes.get(i)[1]="首页->"+Node.realtimes.get(i)[1];
	    	String[] strs=fullname.split("->");
	    	if(strs.length<2)continue;
	    	String channelName="name=首页->"+strs[0]+"->"+strs[1]+",path=";
	    ...
	    {
	    	String fullname=Node.records.get(i)[1];
	    	Node.records.get(i)[1]="首页->"+Node.records.get(i)[1];
	    	String[] strs=fullname.split("->");
	    	if(strs.length<2)continue;
	    	String channelName="name=首页->"+strs[0]+"->"+strs[1]+",path=";
	    	if(!channels.contains(channelName))
	    ...
	    channelMap=DBHelper.saveChannels(categoryId, HOMEPAGE_ID, channels, true);
		List<String[]> allresults=new ArrayList<String[]>();
		allresults.addAll(Node.realtimes);
		allresults.addAll(Node.records);
		DBHelper.saveVideos(categoryId, HOMEPAGE_ID, channelMap, allresults, true);
		//遍历直播
		Node.visiteds.clear();
		Node.channels.clear();
		Node.records.clear();
		...
		for(int i=0;i<channels.size();i++)
		{
		   ...
		   //if(names[1].endsWith("->全能看")||names[1].endsWith("->天翼免费"))
		   {
			   Node.channels.clear();
			   Node.records.clear();
			   Node.realtimes.clear();
